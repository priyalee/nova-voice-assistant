# Nova Voice Assistant

## Demo
Check out all the different versions of Nova:

- **Original version** – [Live Demo](https://original-va.netlify.app/)  
- **Notebook version** – [Live Demo](https://notebook-version.netlify.app/)  
- **Pinkorb version** – [Live Demo](https://pinkorb-nova.netlify.app/)  
- **Black version** – [Live Demo](https://black-nova.netlify.app/)  
- **Color version** – [Live Demo](https://new-va.netlify.app/)  

A browser-based voice recognition assistant built using:

- HTML  
- CSS  
- JavaScript  
- Web Speech API  

---

## Features
- Wake word: **"Nova"**  
- Open websites via voice commands  
- Perform Google searches  
- Speak responses using text-to-speech  
- Responsive and works on most modern browsers  

---

## How to Use
1. Open the webpage in a supported browser (Chrome recommended).  
2. Say **"Nova"** to wake the assistant.  
3. Give commands like:  
   - "Open YouTube"  
   - "Search for cats on Google"  
4. Nova will respond verbally and perform the action.

---

## Optional Improvements / Future Features
- Add more supported commands (weather, Wikipedia search, etc.)  
- Custom wake words  
- Command history log  
- Mobile-friendly interface  

---

## Installation / Contribution
- Clone this repository:  
  ```bash
  git clone <repository-url>
